...
import { SENADO_CANDIDATOS } from '@/lib/types'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (resultado.datos_senado_guardados === true) {
      return NextResponse.json({ exito: false, mensaje: 'Los registros de Senado ya fueron guardados.' })
    }

    if (!resultado.foto_senado) {
      return NextResponse.json({ exito: false, mensaje: 'Debe subir la foto del acta de Senado antes de guardar.' })
    }

    const updateData: Record<string, unknown> = {
      updated_at: new Date().toISOString(),
      datos_senado_guardados: true,
    }

    SENADO_CANDIDATOS.forEach(c => {
    ...
    // Si Cámara ya fue guardada, marcar mesa como completada
    ...
    if (updateError) {
      console.error('Error actualizando senado:', updateError)
      return NextResponse.json({ exito: false, mensaje: 'Error al guardar.' })
    }

    return NextResponse.json({
      exito: true,
      mensaje: `Registros de Senado de Mesa ${mesa_numero} guardados.`,
    })
  } catch (error) {
    console.error('Error guardando senado:', error)
