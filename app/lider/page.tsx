...
export default function LiderPage() {
  ...
  const fetchData = useCallback(async (cedula: string) => {
    setRefreshing(true)
    try {
      const res = await fetch(`/api/lider?cedula=${encodeURIComponent(cedula)}`)
      const data = await res.json()
      if (data.exito) {
        setTestigos(data.testigos)
        setResumen(data.resumen)
        setLiderNombre(data.lider?.nombre || '')
      }
    } catch { /* silent */ }
    setLoading(false)
    setRefreshing(false)
  ...
  useEffect(() => {
    if (authorized && liderCedula) {
      fetchData(liderCedula)
    }
  }, [authorized, liderCedula, fetchData])

  // Auto-refresh + countdown
  useEffect(() => {
    if (!authorized || !liderCedula) return
    const interval = setInterval(() => fetchData(liderCedula), 30000)
  ...
  async function verifyAccess() {
    ...
    try {
      ...
      if (json.exito && json.esLider) {
        setLiderCedula(json.sesion.cedula)
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span className="material-symbols-outlined" style={{ fontSize: '28px', color: '#CE1126' }}>supervisor_account</span>
          </div>
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#94A3B8', fontSize: '13px', fontWeight: 500 }}>Cargando testigos...</p>
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: '0 16px' }}>
        ...
        {/* Stats ribbon */}
        <div className="stats-grid-lider" style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingBottom: '32px' }}>
          {testigos.length === 0 ? (
            <div style={{
              ...
              <p style={{ color: '#94A3B8', fontSize: '14px' }}>No hay testigos asignados.</p>
            </div>
          ) : testigos.map(t => {
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px', flexWrap: 'wrap' }}>
                      {t.mesas.map((m: any) => m.mesa_numero).sort((a: number, b: number) => a - b).map((num: number) => (
                        <span key={num} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px', flexWrap: 'wrap' }}>
                      {t.celular && (
                        <a href={`tel:${t.celular}`} onClick={e => e.stopPropagation()} style={{
                          display: 'inline-flex', alignItems: 'center', gap: '3px',
                          fontSize: '9px', fontWeight: 600, color: '#3B82F6',
                          background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)',
                          borderRadius: '4px', padding: '1px 6px', textDecoration: 'none',
                        }}>
                          <span className="material-symbols-outlined" style={{ fontSize: '10px' }}>call</span>
                          {t.celular}
                        </a>
                      )}
                      {t.correo && (
                        <a href={`mailto:${t.correo}`} onClick={e => e.stopPropagation()} style={{
                          display: 'inline-flex', alignItems: 'center', gap: '3px',
                      ...
                      {t.celular && (
                        <a
                          ...
                          style={{
                            ...
                            background: '#25D366', borderRadius: '4px',
                            padding: '2px 5px', textDecoration: 'none', flexShrink: 0,
                          }}
                        >
                ...
                {isExpanded && (
                  <div style={{ padding: '0 16px 16px', borderTop: '1px solid #F3F4F6' }}>
                    ...
                    <div style={{
                      ...
                      padding: '10px 0', fontSize: '11px', color: '#64748B',
                    }}>
                      {t.celular && (
                        <a href={`tel:${t.celular}`} style={{ color: '#3B82F6', textDecoration: 'none', fontWeight: 600 }}>
                          Tel: {t.celular}
                        </a>
                      )}
                      {t.correo && (
                        <a href={`mailto:${t.correo}`} style={{ color: '#3B82F6', textDecoration: 'none', fontWeight: 600 }}>
                          {t.correo}
                        </a>
                      )}
                    </div>
